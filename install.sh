...
if [[ -r /etc/os-release ]]; then
    ...
    OS_ID="${ID:-}"
    OS_LIKE="${ID_LIKE:-}"
    OS_PRETTY="${PRETTY_NAME:-${NAME:-Unknown Linux}}"
fi

if [[ -z "$DISTRO_MODE" ]]; then
    id_lower="${OS_ID,,}"
    like_lower=" ${OS_LIKE,,} "
    if [[ "$id_lower" == "ubuntu" || "$like_lower" == *" ubuntu "* ]]; then
        DISTRO_MODE="Ubuntu"
    elif [[ "$id_lower" == "arch" || "$id_lower" == "manjaro" || "$id_lower" == "endeavouros" || "$like_lower" == *" arch "* ]]; then
        DISTRO_MODE="Arch"
    elif [[ "$id_lower" == "fedora" || "$like_lower" == *" fedora "* ]]; then
        DISTRO_MODE="Fedora"
    else
        DISTRO_MODE="Fedora"
...
echo "Normal ExeBridge launches and user-local ExeBridge updates do not use sudo or pkexec."
echo

install_fedora() {
    command -v dnf >/dev/null 2>&1 || {
        echo "ERROR: Fedora mode requires dnf, but dnf was not found." >&2
        exit 1
    }
    ...
    echo
    echo "Installing Fedora containment + 32-bit graphics compatibility packages…"
    ...
    if [[ "$(uname -m)" == "x86_64" ]]; then
        sudo dnf install -y \
            ...
            vulkan-loader.i686 \
            || echo "WARNING: One or more Fedora 32-bit graphics packages were unavailable; ExeBridge will still install."
    fi
}

install_ubuntu() {
    command -v apt-get >/dev/null 2>&1 || {
        echo "ERROR: Ubuntu mode requires apt-get, but apt-get was not found." >&2
        exit 1
    }

    if [[ "$(uname -m)" == "x86_64" ]] && command -v dpkg >/dev/null 2>&1; then
        if ! dpkg --print-foreign-architectures | grep -qx i386; then
            echo "Enabling Ubuntu i386 packages for 32-bit Windows/Vulkan compatibility…"
    ...
    if [[ "$(uname -m)" == "x86_64" ]]; then
        echo
        echo "Installing Ubuntu 32-bit Vulkan compatibility packages…"
        sudo apt-get install -y \
            libvulkan1:i386 \
            mesa-vulkan-drivers:i386 \
            || echo "WARNING: Ubuntu 32-bit Vulkan packages were unavailable. NVIDIA users may need the matching :i386 NVIDIA library package for their installed driver."
    fi
}

install_arch() {
    command -v pacman >/dev/null 2>&1 || {
        echo "ERROR: Arch mode requires pacman, but pacman was not found." >&2
        exit 1
    }
    ...
    if [[ "$(uname -m)" == "x86_64" ]]; then
        echo
        echo "Installing Arch 32-bit Vulkan/GameMode loaders…"
        if ! sudo pacman -S --needed --noconfirm lib32-vulkan-icd-loader lib32-gamemode; then
            echo "WARNING: Arch multilib packages could not be installed."
            echo "Enable the [multilib] repository in /etc/pacman.conf, run 'sudo pacman -Syu', then install lib32-vulkan-icd-loader and the 32-bit driver for your GPU."
        fi
        echo "Arch GPU note: install the matching 32-bit Vulkan driver if it is not already present:"
        echo "  AMD:    lib32-vulkan-radeon"
        echo "  Intel:  lib32-vulkan-intel"
        echo "  NVIDIA: lib32-nvidia-utils (matching your installed NVIDIA driver)"
    fi
}
...
# Store the detected/selected distro mode without wiping the user's app settings.
EXEBRIDGE_DISTRO_MODE="$DISTRO_MODE" EXEBRIDGE_DISTRO_EXPLICIT="$MODE_EXPLICIT" python3 - <<'PY'
import json
import os
